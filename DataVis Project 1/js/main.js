  ...
  if(attribute2Value == 'poverty_perc') {
    ...
    attributeText2 = "none";
      ...
      d3.selectAll('.legend-btn:not(.inactive)').each(function() {
        selectedRegion.push(d3.select(this).attr('urban-rural-status'));
      });
    
      if(document.querySelector('#Graph_Type').value == "bar_graph"){
        barchart.data = countyData.filter(d => selectedRegion.includes(d.urban_rural_status)); 
        barchart.updateVis(document.querySelector('#Attribute1').value, attributeText1);
        barchart2.data = countyData.filter(d => selectedRegion.includes(d.urban_rural_status));
        ...
        scatterplot.data = countyData.filter(d => selectedRegion.includes(d.urban_rural_status));
